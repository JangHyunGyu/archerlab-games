...
export class UI {
  ...
  renderBoard(grid, selected = null, fallMoves = []) {
    ...
    this.spawnLandingEffects(fallMoves);
  }
  ...
  spawnLandingEffects(fallMoves = []) {
    if (!fallMoves?.length) return;
    const layerRect = this.refs.particleLayer.getBoundingClientRect();
    const seen = new Set();
    for (const move of fallMoves) {
      const to = move?.to;
      if (!to) continue;
      const key = `${to.row}:${to.col}`;
      if (seen.has(key)) continue;
      seen.add(key);
      const point = this.getCellCenter({ row: to.row, col: to.col }, layerRect);
      if (!point) continue;
      const timing = getFallTiming(move);
      const vfx = document.createElement('span');
      vfx.className = 'landing-vfx';
      vfx.style.left = `${point.x}px`;
      vfx.style.top = `${point.y}px`;
      vfx.style.width = `${point.size * 1.38}px`;
      vfx.style.height = `${point.size * 1.38}px`;
      vfx.style.setProperty('--landing-delay', `${Math.max(0, timing.delay + timing.duration * 0.62)}ms`);
      vfx.style.setProperty('--landing-scale', String(Math.min(1.45, 0.9 + timing.distance * 0.08)));
      this.refs.particleLayer.appendChild(vfx);
      setTimeout(() => vfx.remove(), timing.delay + timing.duration + 700);
    }
    this.trimParticleLayer(window.matchMedia('(max-width: 640px)').matches ? 180 : 360);
  }

  getCellCenter(cell, layerRect) {
    const pixiPoint = this.pixi?.getCellMetrics?.(cell, layerRect);
    if (pixiPoint) {
      return {
        ...pixiPoint,
        type: cell.type || pixiPoint.type || null
      };
    }

    const boardRect = this.refs.board.getBoundingClientRect();
    const style = getComputedStyle(this.refs.board);
    const gap = parseFloat(style.columnGap || style.gap || '0') || 0;
    const padLeft = parseFloat(style.paddingLeft || '0') || 0;
    const padTop = parseFloat(style.paddingTop || '0') || 0;
    const size = 8;
    const cellW = (boardRect.width - padLeft * 2 - gap * (size - 1)) / size;
    const cellH = (boardRect.height - padTop * 2 - gap * (size - 1)) / size;
    return {
      x: boardRect.left - layerRect.left + padLeft + cell.col * (cellW + gap) + cellW / 2,
      y: boardRect.top - layerRect.top + padTop + cell.row * (cellH + gap) + cellH / 2,
