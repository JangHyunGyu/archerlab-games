...
METRICS_JS = ROOT / "js" / "utils" / "CombatVfxMetrics.js"
CANVAS_SIZE = 512
CONTENT_SIZE = 472
ICON_SIZE = 128
FRAME_COUNT = 6
...
def write_visible_metrics(peaks: dict[str, Image.Image]) -> None:
    entries: list[str] = []
    for target in TARGETS:
        bbox = meaningful_bbox(peaks[target.name], threshold=12)
        if not bbox:
            raise ValueError(f"cannot measure empty effect: {target.name}")
        left, top, right, bottom = bbox
        identity = f"{'basic_attack' if target.basic else 'char_skill'}_{target.name}"
        entries.append(
            f"    '{identity}': Object.freeze({{ left: {left}, top: {top}, "
            f"right: {right}, bottom: {bottom}, width: {right - left}, height: {bottom - top} }}),"
        )
    ...
    METRICS_JS.write_text(source, encoding="utf-8", newline="\n")
